...
connectDatabase(){
    ...
    sqlplus -s SYS/Oradoc_db1@'(DESCRIPTION = (ADDRESS = (PROTOCOL = TCP)(HOST = 192.168.5.15)(PORT=1521))(CONNECT_DATA = (SERVICE_NAME=ORCLPDB1.localdomain)))' as SYSDBA <<-EOF>>$LOG_FILE
    @$1 $2;
    exit;
EOF
...
LOG_FILE=$SCRIPT_DIR/${LOG_DIR}/ADMIN_LOGS/$(basename $0 .sh)_$(basename $1 .sql)_$(date +%d%m%y%H%M%S).log
...
if [ ! -d $SCRIPT_DIR/${LOG_DIR}/ADMIN_LOGS ]; then
  mkdir $SCRIPT_DIR/${LOG_DIR}/ADMIN_LOGS
fi
